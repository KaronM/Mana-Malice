...
func activate():
	...
	if mouse_captured and raycast.is_colliding() and !isTalking:  # Only allow shooting when cursor is locked
		if hit and hit is Character and hit.has_method("interact") and hit.talkable: #if interacted with character
			#toggle_dialogue_on()
			if hit.dialogueResource == null:
				push_error("Missing dialogue resource on: " + hit.name)
				return
			var balloon = DialogueManager.show_dialogue_balloon(hit.dialogueResource, hit.currentTitle)
			await balloon.ready
