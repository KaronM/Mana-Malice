...
func submit_response():
	...
	print("Suspect Aggression: ", suspect.aggression, " Suspect Compliance: ", suspect.compliance)
	
	response = trimmed_text
	response_submitted.emit(suspect.suspects[suspect.current_suspect_name],response,suspect.aggression,suspect.compliance, interrogation.current_focus, interrogation.questionsLeft, suspect.currentMood, interrogation.unfocusedStreak)
